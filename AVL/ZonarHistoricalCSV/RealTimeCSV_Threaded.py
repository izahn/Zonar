...
class MainThread(threading.Thread):
    while True:
        ...
        ##    sewer_url = "http://col2225.zonarsystems.net/interface.php?action=showposition&logvers=3.6&username=gisadmins&password=admin1234&operation=path&format=csv&version=2&location=Sewer%20-%20WWTP" + "&starttime=" + tstarttime + "&endtime=" + tendtime
...
if __name__ == '__main__':
    mainThread = MainThread(name = "MainT")
    auxThread = AuxThread(name = "AuxT")
    auxThread.start()
    time.sleep(1)
    mainThread.start()
...
def run():
    buttonrun.config(state=DISABLED)
    buttonstop.config(state=ACTIVE)
    runthread = Thread(target=MainThread)
    runthread.start()

def stop():
    buttonrun.config(state=ACTIVE)
...
root = Tk()
buttonrun = Button(root, text="Run", command=run, state=ACTIVE)
buttonstop = Button(root, text="Stop", command=stop, state=DISABLED)
root.title("Real Time CSV")
